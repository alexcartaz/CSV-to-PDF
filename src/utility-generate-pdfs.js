 ...
 */
const createNewFolder = function(currentPath){
...
const generatePDFs = async function (
  ...
  for(const clientName of Object.keys(quickbooksData)){
    ...
    for(const monthYear of Object.keys(quickbooksData[clientName])){
      ...
      for (const mainJobcode of Object.keys(projects)) {

        // for each arbitrary sub jobcode (eg City of Atlanta)
        for (const subJobcode of Object.keys(projects[mainJobcode])) {
